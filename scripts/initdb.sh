...
psql -v ON_ERROR_STOP=1 --username "$POSTGRES_USER" --dbname "$POSTGRES_DB" <<-EOSQL
    ...
    GRANT ALL PRIVILEGES ON SCHEMA public TO "$COOKY_USER";
    GRANT ALL PRIVILEGES ON ALL TABLES IN SCHEMA public TO "$COOKY_USER";
    GRANT ALL PRIVILEGES ON ALL FUNCTIONS IN SCHEMA public TO "$COOKY_USER";
    GRANT ALL PRIVILEGES ON ALL PROCEDURES IN SCHEMA public TO "$COOKY_USER";
    GRANT ALL PRIVILEGES ON ALL ROUTINES IN SCHEMA public TO "$COOKY_USER";
    GRANT ALL PRIVILEGES ON ALL SEQUENCES IN SCHEMA public TO "$COOKY_USER";
EOSQL
