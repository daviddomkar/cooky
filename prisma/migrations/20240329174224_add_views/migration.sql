...
SELECT recipes.*, users.username as author_username, coalesce(avg(number_of_stars), 0) as average_stars
FROM recipes
...
SELECT recipes.*, users.username as author_username, count(comments.id) as comment_count
FROM recipes
JOIN users ON recipes.author_id = users.id
...
SELECT recipes.*, users.username as author_username, count(recipe_lists.recipe_id) as save_count
FROM recipes
JOIN users ON recipes.author_id = users.id
...
AS $$
BEGIN
    RETURN QUERY SELECT recipes.*, users.username as author_username
    FROM recipes
    JOIN users ON recipes.author_id = users.id
